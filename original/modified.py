...
embedding_dim = 100  # Adjust based on the GloVe model you use
...
X_structural_features = np.concatenate((X_text_structure, X_url_structure), axis=1)
...
X_structural_train, X_structural_test, _, _ = train_test_split(X_structural_features, y, test_size=0.2, random_state=42)
...
input_text_structure = Input(shape=(X_structural_features.shape[1],), name='text_structure_input')
...
embedding = Embedding(input_dim=len(tokenizer.word_index)+1, output_dim=embedding_dim, input_length=500, weights=[embedding_matrix], trainable=False)(input_content)
...
concatenated_inputs = Concatenate()([flattened_layer, input_text_structure])
...
# Define the model
model = Model(inputs=[input_content, input_text_structure], outputs=output)

model.compile(loss='sparse_categorical_crossentropy', optimizer='adam', metrics=['accuracy'])

# Train the model
model.fit(
    x=[X_train, X_structural_train],
    y=y_train,
    batch_size=32,
    epochs=5,
    validation_data=(
        [X_test, X_structural_test],
        y_test
    )
)

# Evaluate the model
loss, accuracy = model.evaluate([X_test, X_structural_test], y_test)
print(f'Test Loss: {loss}, Test Accuracy: {accuracy}')

model.summary()

# Make predictions on the test set
predictions = model.predict([X_test, X_structural_test])
